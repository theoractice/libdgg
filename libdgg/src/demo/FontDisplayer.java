...
public class FontDisplayer extends JComponent {
	...
	public String helloIDS = "⿱宀⿱珤⿲來貝鼠";
	...
	public void paint(Graphics g) {
		...
		draw佈景(g);
		// fdb.drawComponent(g2,tmp字,512, 九宮左上,
		// 九宮左上,Color.ORANGE,FontDB.圓體,true);//過年用
		//fdb.drawComponent(g2, tmp字, this.getHeight()-40, 九宮左上, 九宮左上, 字色, 目前字體, true);
		g2.drawImage(getFontImage(this.getHeight()-80), 九宮左上, 九宮左上,this);
	}

	public BufferedImage getFontImage(int size) {

		BufferedImage img = new BufferedImage(size+40, size+40,
				BufferedImage.TYPE_INT_ARGB);
		Graphics2D g2 = (Graphics2D) img.getGraphics();
		fdb.drawComponent(g2, helloIDS, size, 0, 0, 字色, 目前字體, true);
		fdb.get_charhash(helloIDS);
		g2.dispose();
		return img;
